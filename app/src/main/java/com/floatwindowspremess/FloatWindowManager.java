...
import com.floatwindowspremess.rom.RomUtils;
...
public class FloatWindowManager {
    ...
    private AVCallFloatView floatView;
    ...
    public void applyOrShowFloatWindow(Context context) {
        ...
        if (checkPermission(context)) {
            showWindow(context);
            Toast.makeText(context, "悬浮窗的展示界面", Toast.LENGTH_SHORT).show();
        } else {
    ...
    private boolean checkPermission(Context context) {
        ...
        if (Build.VERSION.SDK_INT < 23 && Build.VERSION.SDK_INT > 19) {
            return RomUtils.surePermission(context);
    ...
    private void applyPermission(Context context) {
        if (Build.VERSION.SDK_INT < 23) {
            if (RomUtils.checkIsMiuiRom()) {
                ...
                ROMSmartisanPermissionApply(context);
    ...
    private void ROMSmartisanPermissionApply(final Context context) {
        showConfirmDialog(context, new OnConfirmResult() {
            @Override
            public void confirmResult(boolean confirm) {
                if (confirm) {
                    RomUtils.applySmartisanosPermission(context);
                } else {
                    Log.e(TAG, "ROM:360, user manually refuse OVERLAY_PERMISSION");
                }
            }
    ...
    private void huaweiROMPermissionApply(final Context context) {
        showConfirmDialog(context, new OnConfirmResult() {
            ...
            public void confirmResult(boolean confirm) {
                if (confirm) {
                 RomUtils.huaWeiApplyPermission(context);
                } else {
    ...
    private void vivoROMPermissionApply(final Context context) {
        showConfirmDialog(context, new OnConfirmResult() {
            ...
            public void confirmResult(boolean confirm) {
                if (confirm) {
                   RomUtils.vivoApplyPermission(context);
                } else {
                    Log.e(TAG, "ROM:huawei, user manually refuse OVERLAY_PERMISSION");
                }
            }
    ...
    private void meizuROMPermissionApply(final Context context) {
        showConfirmDialog(context, new OnConfirmResult() {
            ...
            public void confirmResult(boolean confirm) {
                if (confirm) {
                   RomUtils.meiZuApplyPermission(context);
                } else {
